            ...
            <div className="container px-5 py-20 mx-auto font-theme md:px-10 xl:px-20 ">
                <div className="grid grid-flow-row gap-8 sm:col-span-3 md:grid-flow-col lg:grid-cols-2">
                    <div className="col-span-2 md:col-span-1">
                        <h3 className="mb-2 text-3xl font-bold">{lang['about_headline_title']}</h3>
                        <p className="text-justify ">
                            <span className="opacity-70">
                                {lang['about_headline_subtitle']} 📸, 🚴🏻‍♂️️, 🌍, 👨🏻‍💻.
                            </span>
                        </p>
                        <p className="mt-2 opacity-40">{lang['about_footnote_text']} <a href="https://dribbble.com/shots/14572884-Redesigning-my-portfolio-Webflow" target="_blank" rel="noreferrer" className="underline"> {lang['about_footnote_link']}</a>.</p>
                    </div>
                    <div className="flex justify-end space-x-2">
                        ...
                        <button onClick={() => { scrollInto(refs.top); }} className="flex items-center justify-center w-12 h-12 font-medium text-white bg-gray-700 rounded-md md:mt-12 lg:mt-0 md:justify-self-end focus:outline-none focus:ring-opacity-50 focus:ring-4 focus:ring-pink-500" aria-label={lang['about_cta_navigation']}>
                            <img src={iconArrowTop} className="w-4 h-4 filter-invert" alt={lang['about_cta_navigation']} />
                        </button>
                    </div>
                </div>
                <div className="relative flex flex-wrap items-center mt-12 mb-6 uppercase">
                    ...
                    <div className="absolute right-0 flex mt-2 text-xs opacity-40">
                        ...
                        <span className="hidden md:inline">
                            {
                                Object.keys(translations).filter(t => t !== language).map((t, i) => (
                                    <a className="ml-1" rel="canonical" hrefLang={t} key={i} href={`/${t}`}>{translations[t].language_label}</a>
                                ))
                            }
                        </span>
                    </div>
