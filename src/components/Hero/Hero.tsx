    ...
    return (
        <div className="pt-20">
            ...
            <h1 className="text-4xl font-medium text-gray-900 dark:text-gray-200 sm:text-6xl md:text-7xl lg:text-8xl">
                <span className="font-semibold text-black dark:text-white">Louis Grasset</span> <span className="invisible block text-sm md:visible md:inline-block md:text-7xl lg:text-8xl">—</span> {lang['hero_title']}<br />
            </h1>
            <p className="my-8 text-gray-500 font-theme text-md"><span className="mr-2">{lang['hero_position']} <a href="https://dashlane.com" className="underline"><img className="inline h-6 -mt-1.5 mx-1.5 dark:filter-brightness-100" src={company} alt="Dashlane" /></a> &amp; freelance.</span><img src={iconPin} alt="" aria-hidden="true" className="inline dark:filter-invert w-3.5 h-3.5 -mt-0.5 opacity-30" /> Paris, France.</p>
            <div className="grid max-w-sm grid-rows-2 gap-4 sm:max-w-md sm:grid-rows-none sm:grid-cols-2">
                <button onClick={toggleContact} className="block h-12 px-6 font-medium text-white uppercase bg-gray-900 rounded-md shadow-md dark:bg-white dark:text-gray-900 focus:ring-opacity-50 focus:outline-none focus:ring-4 focus:ring-pink-500">
                    {lang['hero_cta_primary']}&nbsp;&nbsp;<span role='img' aria-label="smiley">🙂</span>
                </button>
